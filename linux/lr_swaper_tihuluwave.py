...
THEME_NAMES = (THEME_TIHULUWAVE, THEME_PLAIN)
...
def theme_palette(name=None):
    """Return the current UI color palette.

    Plain Theme is the conservative classic dark interface.
    Tihuluwave Theme is the neon/vaporwave interface.
    """
    name = name if name in THEME_NAMES else THEME_TIHULUWAVE
    if name == THEME_PLAIN:
        return {
            "name": THEME_PLAIN,
            "bg": "#111418",
...
def remove_swap_preserving_settings():
    """Turn off L/R swap while preserving target, EQ, balance, and volume."""
    state = load_state()
    preserved_volume = clamp_system_volume(state.get("system_volume", get_current_output_volume_percent()))
    bass_db = clamp_db(state.get("bass_db", 0.0))
    treble_db = clamp_db(state.get("treble_db", 0.0))
    balance_value = clamp_balance(state.get("balance_value", 50))
    ...
    if not target or not sink_exists(target) or is_our_sink(target):
        return False, "Could not find a real target device to turn swap off."
    ...
    state2 = load_state()
    state2["system_volume"] = preserved_volume
    state2["balance_value"] = balance_value
    state2["balance_enabled"] = abs(balance_value - 50.0) > 0.5
    state2["bass_db"] = 0.0
    state2["treble_db"] = 0.0
    state2["bass_enabled"] = False
    state2["treble_enabled"] = False
    state2["mode"] = "A"
    save_state(state2)

    set_sink_raw_volume(target, preserved_volume, preserved_volume)
    ok, msg = set_default_and_move(target)
    if not ok:
        return False, msg

    if abs(bass_db) > 0.05 or abs(treble_db) > 0.05:
        ok2, msg2 = apply_tone_values(bass_db, treble_db, base_override=target)
        if not ok2:
            return False, "Swap turned off, but EQ restore failed: " + msg2
        msg = msg + " " + msg2
    else:
        set_sink_volume_with_balance(target, preserved_volume, balance_value)

    apply_balance_value(balance_value)

    state3 = load_state()
    state3["system_volume"] = preserved_volume
    state3["balance_value"] = balance_value
    state3["balance_enabled"] = abs(balance_value - 50.0) > 0.5
    state3["mode"] = "A"
    save_state(state3)
    return True, f"Swap turned off. EQ/balance preserved on {sink_descriptions().get(target, target)}."
...
class VolumeStyleSlider(tk.Frame):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.palette = theme_palette(load_theme_name())
        super().__init__(master, bg=self.palette["panel"], **kwargs)
        self.text = text
    ...
    def apply_theme(self, palette):
        ...
        try:
            ...
            self.name_label.configure(bg=palette["panel"], fg=palette["fg"])
            ...
            self.detail_label.configure(bg=palette["panel"], fg=palette["subtle"])
            self.draw()
        except Exception:
            pass
    ...
    def draw(self):
        ...
        c.create_line(left, y, right, y, fill=self.palette["slider_track"], width=4)
        c.create_line(left, y, x, y, fill=self.palette["accent"], width=4)

        if self.to == 100 and self.from_ <= 50 <= self.to:
            cx = left + ((50 - self.from_) / (self.to - self.from_)) * (right - left)
            c.create_line(cx, y - 9, cx, y + 9, fill=self.palette["subtle"], width=2)

        r = 10
        c.create_oval(x - r, y - r, x + r, y + r, fill=self.palette["accent"], outline=self.palette["accent"])
...
class GlowButton(tk.Canvas):
    """Canvas based rounded/glow button for the modern Tihuluwave UI."""

    def __init__(self, master, text, command=None, palette=None, active=False, width=128, height=66, **kwargs):
        self.palette = palette or theme_palette(load_theme_name())
        ...
        self.bind("<Configure>", lambda _event=None: self.draw())
    ...
    def draw(self):
        ...
        if self.active:
            ...
            fill = "#241851" if p["name"] == THEME_TIHULUWAVE else "#2b333c"
            outline = p["accent"]
            text_color = p["fg"]
            glow = p["accent"]
        else:
        ...
        # Fake glow: several nested outlines. Tk canvas has no real blur, but
        # this gives a softer neon edge than rectangular native buttons.
        if self.active or self._hover:
            self.rounded_rect(3, 3, w - 3, h - 3, 14, fill="", outline=glow, width=1)
            self.rounded_rect(5, 5, w - 5, h - 5, 13, fill="", outline=glow, width=1)

        self.rounded_rect(8 + offset, 8 + offset, w - 8 + offset, h - 8 + offset, 12, fill=fill, outline=outline, width=2)

        lines = str(self.text_value).split("\n")
        total = (len(lines) - 1) * 18
        cy = h / 2 - total / 2 + offset
        for i, line in enumerate(lines):
            font = ("Sans", 11, "bold") if i == len(lines) - 1 else ("Sans", 13, "bold")
            self.create_text(w / 2 + offset, cy + i * 18, text=line, fill=text_color, font=font, anchor="center")
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.theme_name = tk.StringVar(value=load_theme_name())
        self.palette = theme_palette(self.theme_name.get())
        self.target_text = tk.StringVar(value=f"Target: {get_target_description()}")
        self.route_status_text = tk.StringVar(value="● Virtual Route: Inactive")
        self.swap_status_text = tk.StringVar(value="● Swap: Off")
        self.eq_status_text = tk.StringVar(value="● EQ: Off")
        self._tone_after_id = None
        ...
        self._last_help_message = None
        self._preset_badges = []

        # General right-side scrollbar for the whole app.
        self.scroll_canvas = tk.Canvas(self, borderwidth=0, highlightthickness=0, bg=self.palette["bg"])
        ...
        # ------------------------------------------------------------------
        # Modern dashboard layout. All classic controls are still present; only
        # the placement has changed to match the newer macOS-style mockup.
        # ------------------------------------------------------------------
        header = ttk.Frame(self.page)
        header.grid(row=0, column=0, sticky="ew", pady=(0, 12))
        header.columnconfigure(1, weight=1)

        logo = ttk.Label(header, text="〰", style="Logo.TLabel")
        logo.grid(row=0, column=0, rowspan=2, sticky="w", padx=(2, 12))
        ttk.Label(header, text="L/R Swaper", style="Title.TLabel").grid(row=0, column=1, sticky="w")
        ttk.Label(header, text=f"v{VERSION} • Linux", style="Subtle.TLabel").grid(row=1, column=1, sticky="w", pady=(2, 0))

        top_actions = ttk.Frame(header)
        top_actions.grid(row=0, column=2, rowspan=2, sticky="e")
        ttk.Button(top_actions, text="Plain Theme", command=lambda: self.switch_theme_mode(THEME_PLAIN), style="Icon.TButton").grid(row=0, column=0, padx=(0, 6))
        ttk.Button(top_actions, text="Tihuluwave Theme", command=lambda: self.switch_theme_mode(THEME_TIHULUWAVE), style="ActiveTheme.TButton").grid(row=0, column=1, padx=(0, 10))
        ttk.Button(top_actions, text="Help", command=self.show_help_dialog, style="Icon.TButton").grid(row=0, column=2, padx=(0, 6))
        ttk.Button(top_actions, text="Diag.", command=self.show_diagnostics, style="Icon.TButton").grid(row=0, column=3, padx=(0, 6))
        ttk.Button(top_actions, text="Collapse", command=self.collapse_all_sections, style="Icon.TButton").grid(row=0, column=4, padx=(0, 6))
        ttk.Button(top_actions, text="Expand", command=self.expand_all_sections, style="Icon.TButton").grid(row=0, column=5)

        routing = ttk.LabelFrame(self.page, text="↔  ROUTING & SIGNAL", style="Card.TLabelframe", padding=(14, 12))
        routing.grid(row=1, column=0, sticky="ew", pady=(0, 12))
        routing.columnconfigure(0, weight=2)
        routing.columnconfigure(1, weight=0)
        routing.columnconfigure(2, weight=1)
        routing.rowconfigure(1, weight=1)

        ttk.Label(routing, text="Output Device", style="CardTitle.TLabel").grid(row=0, column=0, sticky="w", pady=(0, 8))

        columns = ("default", "kind", "output", "state")
        tree_wrap = ttk.Frame(routing)
        tree_wrap.grid(row=1, column=0, rowspan=3, sticky="nsew", padx=(0, 10))
        tree_wrap.columnconfigure(0, weight=1)
        tree_wrap.columnconfigure(1, weight=0)
        tree_wrap.rowconfigure(0, weight=1)

        self.tree = ttk.Treeview(tree_wrap, columns=columns, show="headings", height=6, selectmode="browse")
        self.tree.heading("default", text="Default")
        self.tree.heading("kind", text="Type")
        self.tree.heading("output", text="Output")
        self.tree.heading("state", text="State")
        self.tree.column("default", width=72, anchor="center", stretch=False)
        self.tree.column("kind", width=110, anchor="w", stretch=False)
        self.tree.column("output", width=380, anchor="w", stretch=True)
        self.tree.column("state", width=118, anchor="center", stretch=False)
        ...
        route_buttons.grid(row=1, column=1, sticky="n", padx=(8, 16))
        ttk.Button(route_buttons, text="▶  Start", command=self.swap_current_default_gui, style="Start.TButton").grid(row=0, column=0, sticky="ew", pady=(0, 8))
        ttk.Button(route_buttons, text="■  Stop", command=self.disable_gui, style="Stop.TButton").grid(row=1, column=0, sticky="ew", pady=(0, 12))
        ttk.Button(route_buttons, text="↻  Refresh", command=self.refresh, style="Action.TButton").grid(row=2, column=0, sticky="ew", pady=(0, 8))
        ttk.Button(route_buttons, text="Use selected", command=self.use_selected_target_gui, style="Action.TButton").grid(row=3, column=0, sticky="ew")

        signal = ttk.Frame(routing)
        signal.grid(row=1, column=2, rowspan=3, sticky="nsew")
        signal.columnconfigure(0, weight=1)
        ttk.Label(signal, textvariable=self.target_text, style="CardTitle.TLabel", wraplength=340).grid(row=0, column=0, sticky="w", pady=(0, 10))

        status_grid = ttk.Frame(signal)
        status_grid.grid(row=1, column=0, sticky="ew", pady=(0, 8))
        status_grid.columnconfigure(0, weight=1)
        status_grid.columnconfigure(1, weight=1)
        status_grid.columnconfigure(2, weight=1)

        self.route_status_label = ttk.Label(status_grid, textvariable=self.route_status_text, style="InactivePill.TLabel", anchor="center")
        self.route_status_label.grid(row=0, column=0, sticky="ew", padx=(0, 6))
        self.swap_status_label = ttk.Label(status_grid, textvariable=self.swap_status_text, style="InactivePill.TLabel", anchor="center")
        self.swap_status_label.grid(row=0, column=1, sticky="ew", padx=(0, 6))
        self.eq_status_label = ttk.Label(status_grid, textvariable=self.eq_status_text, style="InactivePill.TLabel", anchor="center")
        self.eq_status_label.grid(row=0, column=2, sticky="ew")

        ttk.Label(
            signal,
            text="Linux does not use a real input signal meter here. These labels show routing state only.",
            style="Subtle.TLabel",
            wraplength=340,
        ).grid(row=2, column=0, sticky="w", pady=(6, 0))

        main_grid = ttk.Frame(self.page)
        main_grid.grid(row=2, column=0, sticky="ew")
        main_grid.columnconfigure(0, weight=3)
        main_grid.columnconfigure(1, weight=2)
        main_grid.columnconfigure(2, weight=2)

        sound = ttk.LabelFrame(main_grid, text="▥  SOUND CONTROLS", style="Card.TLabelframe", padding=(14, 12))
        sound.grid(row=0, column=0, sticky="nsew", padx=(0, 10), pady=(0, 12))
        sound.columnconfigure(0, weight=1)
        ...
        self.volume_slider.grid(row=0, column=0, padx=4, pady=(4, 5), sticky="ew")
        ...
        self.bass_slider = VolumeStyleSlider(
            ...
            from_=0,
            to=100,
            value_formatter=lambda value: f"{tone_slider_value_to_db(value):+.1f} dB",
        )
        self.bass_slider.grid(row=1, column=0, padx=4, pady=5, sticky="ew")
        ...
        self.treble_slider.grid(row=2, column=0, padx=4, pady=5, sticky="ew")
        ...
        self.balance_slider.grid(row=3, column=0, padx=4, pady=(5, 8), sticky="ew")

        sound_footer = ttk.Frame(sound)
        sound_footer.grid(row=4, column=0, sticky="ew", pady=(6, 0))
        sound_footer.columnconfigure(0, weight=1)
        ttk.Button(sound_footer, text="↻ Reset (Neutral)", command=self.neutral_gui, style="Accent.TButton").grid(row=0, column=0, sticky="w")
        ttk.Label(sound_footer, text="✓ Neutral keeps volume unchanged", style="Success.TLabel").grid(row=0, column=1, sticky="e", padx=(10, 0))

        quick = ttk.LabelFrame(main_grid, text="⚡  QUICK ACTIONS", style="Card.TLabelframe", padding=(12, 10))
        quick.grid(row=0, column=1, sticky="nsew", padx=(0, 10), pady=(0, 12))
        ...
        self.swap_btn = GlowButton(quick, "⇄\nSwap L/R", command=lambda: self.toggle_swap_push_button("A"), palette=self.palette)
        self.swap_btn.grid(row=0, column=0, sticky="nsew", padx=(0, 8), pady=(0, 8))
        self.alt_swap_btn = GlowButton(quick, "⇆\nAlt Swap", command=lambda: self.toggle_swap_push_button("B"), palette=self.palette)
        self.alt_swap_btn.grid(row=0, column=1, sticky="nsew", pady=(0, 8))

        set_default_btn = GlowButton(quick, "★\nSet Default", command=self.set_selected_default_gui, palette=self.palette)
        set_default_btn.grid(row=1, column=0, sticky="nsew", padx=(0, 8), pady=(0, 8))
        fix_btn = GlowButton(quick, "↪\nFix Streams", command=self.fix_now_gui, palette=self.palette)
        fix_btn.grid(row=1, column=1, sticky="nsew", pady=(0, 8))

        diag_btn = GlowButton(quick, "🛠\nDiagnostics", command=self.show_diagnostics, palette=self.palette)
        diag_btn.grid(row=2, column=0, sticky="nsew", padx=(0, 8))
        disable_btn = GlowButton(quick, "⏻\nDisable", command=self.disable_gui, palette=self.palette)
        disable_btn.grid(row=2, column=1, sticky="nsew")

        self.quick_action_buttons.extend([self.swap_btn, self.alt_swap_btn, set_default_btn, fix_btn, diag_btn, disable_btn])

        test = ttk.LabelFrame(main_grid, text="🎧  L / R TEST", style="Card.TLabelframe", padding=(14, 12))
        test.grid(row=0, column=2, sticky="nsew", pady=(0, 12))
        test.columnconfigure(0, weight=1)
        ttk.Button(test, text="Ⓛ  Left", command=lambda: self.play_test_gui("left"), style="Wide.TButton").grid(row=0, column=0, sticky="ew", pady=(0, 10))
        ttk.Button(test, text="Ⓡ  Right", command=lambda: self.play_test_gui("right"), style="Wide.TButton").grid(row=1, column=0, sticky="ew", pady=(0, 10))
        ttk.Button(test, text="Ⓛ Ⓡ  Both", command=lambda: self.play_test_gui("lr"), style="Wide.TButton").grid(row=2, column=0, sticky="ew", pady=(0, 12))
        ttk.Label(test, text="After swap, Left should come from the right speaker.", style="Subtle.TLabel", wraplength=220).grid(row=3, column=0, sticky="ew")

        presets = ttk.LabelFrame(self.page, text="▱  PRESETS", style="Card.TLabelframe", padding=(14, 12))
        presets.grid(row=3, column=0, sticky="ew", pady=(0, 12))
        for i in range(3):
            presets.columnconfigure(i, weight=1)

        for i in range(3):
            card = ttk.Frame(presets, style="CardInner.TFrame", padding=(10, 8))
            card.grid(row=0, column=i, sticky="ew", padx=(0 if i == 0 else 8, 0))
            card.columnconfigure(1, weight=1)
            badge_color = [self.palette["accent2"], self.palette["purple"], self.palette["warning"]][i]
            badge = tk.Label(card, text=str(i + 1), bg=badge_color, fg="#ffffff", font=("Sans", 18, "bold"), width=2, height=1)
            badge.grid(row=0, column=0, rowspan=2, padx=(0, 12), pady=(0, 4), sticky="w")
            if not hasattr(self, "_preset_badges"):
                self._preset_badges = []
            self._preset_badges.append(badge)
            ttk.Label(card, text=f"Preset {i+1}", style="CardTitle.TLabel").grid(row=0, column=1, sticky="w")
            ttk.Label(card, text="Volume independent", style="Subtle.TLabel").grid(row=1, column=1, sticky="w")
            ttk.Button(card, text="Save", command=lambda slot=i+1: self.save_settings_gui(slot), style="Accent.TButton").grid(row=2, column=0, columnspan=2, sticky="ew", pady=(8, 4))
            ttk.Button(card, text="Load", command=lambda slot=i+1: self.load_settings_gui(slot), style="Action.TButton").grid(row=3, column=0, columnspan=2, sticky="ew")

        help_info = ttk.LabelFrame(self.page, text="ⓘ  HELP & INFO", style="Card.TLabelframe", padding=(14, 12))
        help_info.grid(row=4, column=0, sticky="ew", pady=(0, 10))
        help_info.columnconfigure(0, weight=1)
        help_info.columnconfigure(1, weight=1)
        help_info.columnconfigure(2, weight=1)
        help_info.columnconfigure(3, weight=1)
        ttk.Label(help_info, text="🔊  Select target output", style="InfoPill.TLabel", anchor="center").grid(row=0, column=0, sticky="ew", padx=(0, 8))
        ttk.Label(help_info, text="▶  Press Start / Swap", style="InfoPill.TLabel", anchor="center").grid(row=0, column=1, sticky="ew", padx=(0, 8))
        ttk.Label(help_info, text="〰  Adjust EQ and L/R", style="InfoPill.TLabel", anchor="center").grid(row=0, column=2, sticky="ew", padx=(0, 8))
        ttk.Label(help_info, text="●  Local only", style="InfoPill.TLabel", anchor="center").grid(row=0, column=3, sticky="ew")

        self._dashboard_sections = [routing, main_grid, presets, help_info]

        self.status = ttk.Label(self.page, anchor="w", wraplength=980, style="Status.TLabel")
        self.status.grid(row=5, column=0, sticky="ew", pady=(2, 0))

        self.update_tone_labels()
        self.refresh()
        self.apply_theme_to_widgets()
        self.update_quick_action_states()
    ...
    def switch_theme_mode(self, name):
        if name not in THEME_NAMES:
            return
        save_theme_name(name)
        if name == THEME_PLAIN:
            self.set_status("Switching to Plain Theme...")
            restart_app()
        else:
            self.set_status("Tihuluwave Theme is already active.")

    def apply_theme_to_widgets(self):
        self.palette = theme_palette(self.theme_name.get())
        try:
            self.configure(bg=self.palette["bg"])
            self.scroll_canvas.configure(bg=self.palette["bg"])
            self.page.configure(style="TFrame")
        except Exception:
            pass
        ...
        for slider_name in ("volume_slider", "bass_slider", "treble_slider", "balance_slider"):
            ...
            if slider is not None:
                try:
                    slider.apply_theme(self.palette)
                except Exception:
                    pass

        for button in getattr(self, "quick_action_buttons", []):
            try:
                button.apply_theme(self.palette)
            except Exception:
                pass

        try:
            colors = [self.palette["accent2"], self.palette["purple"], self.palette["warning"]]
            for badge, color in zip(getattr(self, "_preset_badges", []), colors):
    ...
    def show_help_dialog(self):
        msg = (
            "Quick help\n\n"
            "1. Select a real output device.\n"
            "2. Click Use selected to lock the target.\n"
            "3. Click Start to route audio through L/R Swaper.\n"
            "4. Use Swap L/R or Alt Swap. Click the same swap button again to turn swap off.\n"
            "5. Use Disable or Stop to remove the virtual chain.\n\n"
            "All processing is local on Linux. Use the Theme menu for Plain Theme or Tihuluwave Theme."
        )
        self._last_help_message = msg
        self.set_status(msg.replace("\n", " "))
        try:
            messagebox.showinfo(APP_NAME + " Help", msg)
        except Exception:
            pass

    def _current_active_swap_mode(self):
        state = load_state()
        swapped = state.get("swapped_sink_name")
        if swapped and sink_exists(swapped):
    ...
    def update_status_pills(self):
        ...
        route_active = bool(current and is_our_sink(current))
        swap_active = bool(state.get("swapped_sink_name") and sink_exists(state.get("swapped_sink_name")))
        eq_active = bool(
            (state.get("eq_sink_name") and sink_exists(state.get("eq_sink_name")))
            or abs(float(state.get("bass_db", 0.0) or 0.0)) > 0.05
            or abs(float(state.get("treble_db", 0.0) or 0.0)) > 0.05
        )

        self.route_status_text.set("● Virtual Route: Active" if route_active else "● Virtual Route: Inactive")
        self.swap_status_text.set("● Swap: On" if swap_active else "● Swap: Off")
        self.eq_status_text.set("● EQ: On" if eq_active else "● EQ: Off")

        try:
            self.route_status_label.configure(style="ActivePill.TLabel" if route_active else "InactivePill.TLabel")
            self.swap_status_label.configure(style="ActivePill.TLabel" if swap_active else "InactivePill.TLabel")
            self.eq_status_label.configure(style="ActivePill.TLabel" if eq_active else "InactivePill.TLabel")
        except Exception:
            pass

    def update_quick_action_states(self):
        self.update_status_pills()
        active = self._current_active_swap_mode()
        try:
            self.swap_btn.set_active(active == "A")
            self.alt_swap_btn.set_active(active == "B")
            self.swap_btn.set_text("⇄\nSwap L/R ✓" if active == "A" else "⇄\nSwap L/R")
            self.alt_swap_btn.set_text("⇆\nAlt Swap ✓" if active == "B" else "⇆\nAlt Swap")
        except Exception:
            pass

    def expand_all_sections(self):
        if self._dashboard_sections:
            for widget in self._dashboard_sections:
                try:
                    widget.grid()
                except Exception:
                    pass
            self.set_status("Expanded all sections.")
            return
        ...
        if self._dashboard_sections:
            for widget in self._dashboard_sections:
                try:
                    ...
                    pass
            self.set_status("Collapsed all main sections. Click Expand to restore them.")
            return
        for title, state in self._section_states.items():
            state.set(False)
            body = self._section_bodies.get(title)
    ...
    def toggle_swap_push_button(self, mode):
        mode = str(mode or "A").upper()
        active = self._current_active_swap_mode()

        # Press the already-active mode again to release/deswap.
        if active == mode:
            ok, msg = remove_swap_preserving_settings()
        ...
        if not ok:
            ...
            if active and active != mode:
                self.set_status(f"Switched swap mode to {mode}. Press again to release/deswap. {msg}")
            else:
                self.set_status(f"Swap mode {mode} enabled. Press the active button again to release/deswap. {msg}")

    def toggle_swap_gui(self, mode):
        mode = str(mode or "A").upper()
        active = self._current_active_swap_mode()
        if active == mode:
            ok, msg = remove_swap_preserving_settings()
        ...
        if not ok:
            ...
            if active and active != mode:
                self.set_status(f"Switched swap mode to {mode}. {msg}")
            else:
                self.set_status(f"Swap mode {mode} enabled. Click the same button again to turn it off. {msg}")
    ...
    def setup_style(self):
        try:
            ...
            self.palette = theme_palette(self.theme_name.get() if hasattr(self, "theme_name") else load_theme_name())
            p = self.palette
            ...
            title_color = fg
            card_title_color = p["accent3"]
            if p["name"] == THEME_TIHULUWAVE:
                logo_color = p["accent"]
                button_active = "#30205a"
                top_button = "#1b123a"
            else:
                logo_color = p["accent"]
                button_active = "#2b333c"
                top_button = panel2
            ...
            style.map("TButton", background=[("active", button_active)], foreground=[("active", fg)])

            style.configure("TCombobox", fieldbackground=panel2, background=panel2, foreground=fg, arrowcolor=accent, padding=(6, 4))
            style.map("TCombobox", fieldbackground=[("readonly", panel2)], foreground=[("readonly", fg)])

            style.configure("Section.TButton", background=panel2, foreground=fg, font=("Sans", 11, "bold"), padding=(10, 7), anchor="w")
            style.map("Section.TButton", background=[("active", button_active)], foreground=[("active", fg)])
            style.configure("Icon.TButton", background=top_button, foreground=fg, font=("Sans", 10, "bold"), padding=(10, 7))
            style.configure("ActiveTheme.TButton", background=p["active_bg"], foreground="#ffffff", font=("Sans", 10, "bold"), padding=(10, 7))
            style.configure("Action.TButton", background=panel2, foreground=fg, font=("Sans", 10, "bold"), padding=(10, 8))
            style.configure("Accent.TButton", background=p["accent2"], foreground="#ffffff", font=("Sans", 10, "bold"), padding=(10, 8))
            style.configure("Start.TButton", background=p["start"], foreground="#ffffff", font=("Sans", 13, "bold"), padding=(22, 16))
            style.configure("Stop.TButton", background=p["stop"], foreground="#ffffff", font=("Sans", 12, "bold"), padding=(18, 14))
            style.configure("Tile.TButton", background=panel2, foreground=fg, font=("Sans", 10, "bold"), padding=(12, 14))
            style.configure("ActiveTile.TButton", background=p["active_bg"], foreground="#ffffff", font=("Sans", 10, "bold"), padding=(12, 14))
            style.configure("Wide.TButton", background=panel2, foreground=fg, font=("Sans", 11, "bold"), padding=(12, 12))

            style.configure("TLabelframe", background=bg, foreground=fg, bordercolor=border, relief="solid", padding=(8, 6))
            style.configure("Card.TLabelframe", background=bg, foreground=card_title_color, bordercolor=border, relief="solid", padding=(10, 8))
            style.configure("TLabelframe.Label", background=bg, foreground=fg, font=("Sans", 10, "bold"))
            style.configure("Card.TLabelframe.Label", background=bg, foreground=card_title_color, font=("Sans", 12, "bold"))

            style.configure("Title.TLabel", background=bg, foreground=title_color, font=("Sans", 22, "bold"))
            style.configure("Logo.TLabel", background=bg, foreground=logo_color, font=("Sans", 28, "bold"))
            style.configure("Subtle.TLabel", background=bg, foreground=subtle, font=("Sans", 10))
            style.configure("CardTitle.TLabel", background=bg, foreground=fg, font=("Sans", 11, "bold"))
            style.configure("Success.TLabel", background=bg, foreground=p["success"], font=("Sans", 10, "bold"))
            style.configure("InfoPill.TLabel", background=panel, foreground=fg, font=("Sans", 10, "bold"), padding=(12, 10))
            style.configure("ActivePill.TLabel", background=p["success_bg"], foreground=p["success"], font=("Sans", 10, "bold"), padding=(10, 8))
            style.configure("InactivePill.TLabel", background=panel2, foreground=subtle, font=("Sans", 10, "bold"), padding=(10, 8))
            style.configure("Status.TLabel", background=bg, foreground=fg, font=("Sans", 10))
            style.configure("CardInner.TFrame", background=panel)

            style.configure("Signal.Horizontal.TProgressbar", background=p["success"], troughcolor=panel2, bordercolor=panel2, lightcolor=p["success"], darkcolor=p["success"])
            style.configure("Treeview", background=panel, fieldbackground=panel, foreground=fg, rowheight=28, font=("Sans", 10), borderwidth=0)
    ...
    def refresh(self):
        ...
        self.target_text.set(f"Target: {get_target_description()}")
        self.update_quick_action_states()
        self.set_status("Status: Ready • Select an output, use it as target, then Start/Swap. Presets are volume-independent.")
